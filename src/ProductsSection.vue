...
const productsHome = [
  ...
  },
  {
    ...
    link: "https://www.npmjs.com/",
    image: "tank-war.png",
    bgClass: "bg-[url('assets/IMG_9197.JPG')]"
  },
]
</script>

<template>
  <div class="relative isolate overflow-hidden bg-[url('assets/Hero-bg.png')] h-auto place-content-center p-10 border-none bg-center bg-no-repeat bg-cover">

    <h1 class="mt-2 text-3xl font-bold tracking-tight text-white sm:text-4xl">Our Products</h1>
    <div class="mt-10 grid lg:grid-cols-5 md:grid-cols-2 sm:grid-cols-8 gap-y-8 gap-x-10 max-h-screen" >
      <div v-for="product in productsHome" :key="product.name" class="product-card border-none box-content" >
        <a  href="#" class="h-10 overflow-hidden ">
          <div class="overflow-hidden box-content h-1/2 max-h-1/2 flex ">
            <!--<img :src="'src/assets/' + product.image" :alt="product.name" class="transition-all h-full w-full hover:scale-105 place-self-center"/>-->
          </div>
          <div class="p-10" >
